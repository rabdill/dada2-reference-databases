---
title: "Investigate problematic taxa (Silva 138)"
...
tax %>%
  ...
  c
```
There is just one case of an extra rank being used.
...
```
The vast majority of taxa have the expected number of ranks, but there are 124 with an incorrect number and therefore will not have been handled correctly by the DADA2 formatting function.
In 2 cases, there are too many ranks; these are children of the YM suborder.
```{r}
tax %>% filter(path_ranks > path_ranks_expected) %>% pull(path)
```
The remaining cases where an expected rank is missing.
...
tax0 %>%
  ...
  knitr::kable(digits = 3)
```
The problem is confined to families and genera.

Finally, let's output a table of the affected taxa,
```{r}
not_ok <- tax %>%
  filter(path_ranks != path_ranks_expected) %>%
  select(rank, name, path, ranks_present = path_ranks, ranks_expected = path_ranks_expected) %>%
...
Let's also check the children of the suborder "YM".
```{r}
tax %>%
  filter(str_detect(path, ";YM;")) %>%
  select(name, rank, path) %>%
  mutate(
    across(path, str_extract, "(?<=Bacteria;Patescibacteria;Saccharimonadia;).+"),
    name %in% not_ok$name
  )
```
Just a single family and genus, which are both in "Bad taxa" list.
Let's see if these made it into the formatted training database,
```{r}
# In the Silva NR sequences:
precursor_file <- file.path(path,
  "precursors/SILVA_138_SSURef_NR99_tax_silva.fasta.gz")
cmd <- "zcat {precursor_file} | grep ';YM;' | wc" %>% str_glue
system(cmd)
# In the formatted training files:
db_file <- file.path(path, "output/silva_nr99_v138_train_set.fa.gz")
cmd <- "zcat {db_file} | grep ';YM;' | wc" %>% str_glue
system(cmd)
```
The DADA2 formatting process apparently dropped these sequences, which I'd guess is because it detected there were too many ranks in the path.
